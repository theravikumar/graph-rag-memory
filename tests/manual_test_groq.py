...
# ---------------------------------------------------------
# We disable query_expansion to keep retrieval latency under 50ms
...
def main():
    print("="*60)
    print(f"🚀 Groq-Powered Graph Memory Tester ({MODEL_NAME})")
    print("="*60)
    ...
    while True:
        try:
            ...
            # 2. Always Retrieve Graph Context (Ultra-fast without query expansion)
            ...
            print(f"\n[Groq] (Total: {total_latency:.0f}ms | Generation: {gen_latency:.0f}ms): {response_text}")
